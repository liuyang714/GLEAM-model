...
def predict(
    ...
    if any(l != -1 for l in true_labels):
        ...
        for _ in range(1000):
            ...
            for i in present:
                try:
                    boot_aucs_box[i].append(roc_auc_score(y_onehot[idx_b][:, i], valid_logits[idx_b][:, i]))
                except Exception:
                    pass
        ...
        sns.despine()
        plt.tight_layout()
        auc_box_path = os.path.join(save_dir, "fusion_auc_boxplot.png")
        plt.savefig(auc_box_path, dpi=300)
        plt.close()
        print(f"[OK] AUC boxplot saved to: {auc_box_path}")

        # F1 Violin Plot (bootstrap)
        boot_f1s = {i: [] for i in present}
        for _ in range(1000):
            idx_b = rng_box.randint(0, n, n)
            boot_true_b = np.array(valid_true)[idx_b]
            boot_pred_b = np.array(valid_pred)[idx_b]
            for i in present:
        ...
        sns.despine()
        plt.tight_layout()
        f1_violin_path = os.path.join(save_dir, "fusion_f1_violin.png")
        plt.savefig(f1_violin_path, dpi=300)
        plt.close()
        print(f"[OK] F1 violin saved to: {f1_violin_path}")

        # Summary Excel with bootstrap CI
        summary_path = os.path.join(save_dir, "summary.xlsx")
        save_summary_xlsx(
            summary_path, class_names, valid_true, valid_pred, valid_logits,
            title_text="Fusion Six-Class Results",
        )
    else:
